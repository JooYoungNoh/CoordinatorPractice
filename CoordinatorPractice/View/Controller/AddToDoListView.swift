...
class AddToDoListView: UIViewController {
    weak var coordinator: AddToDoListCoordinator?
    ...
    init(coordinator: AddToDoListCoordinator?, viewModel: ToDoListViewModel) {
...
extension AddToDoListView {
    @objc func addToDoData(_ sender: UIButton) {
        if titleTextView.text != "" && descriptionTextView.text != "" {
            self.viewModel.saveTodo(title: titleTextView.text, description: descriptionTextView.text)
            coordinator?.delegate?.reloadData()
            coordinator?.dismiss()
        } else {
